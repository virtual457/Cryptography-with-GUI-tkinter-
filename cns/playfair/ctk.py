...
enc=[]
def show():
	enc=encrypt(e1.get(),e2.get())
	e="".join(map(str,enc))
	blank.insert(0,e)
master=Tk()
master.title("playfair encryptor")
First=Label(master,text="your name").grid(row=0,column=0)
Last=Label(master,text="message to be passed").grid(row=1,column=0)
